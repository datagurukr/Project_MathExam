  ...
  <h1 class="page-header">가입 승인</h1>
            ...
            <div class="col-sm-3 text-right">
                <div class="form-group">
                  <div class="form-control">
                    <p class="text-info">
                        대기: <? echo number_format($out_cnt); ?> 명
                    </p>
                  </div>
                </div>
            </div>
    ...
    <table class="table table table-bordered text-center">
      <thead>
        ...
        $sort = $protocol.'://'.$_SERVER["HTTP_HOST"].'/admin/join?';
        $order = '';
        if ( isset($_GET['order']) ) {
            if ( $_GET['order'] == 'asc' ) {
                $order = 'desc';                
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <a href="/admin/join/editor/<? echo $row['user_id']; ?>/approval" class="btn btn-primary btn-xs">수락</a>  
          </td>
          <td>
            <a href="/admin/join/editor/<? echo $row['user_id']; ?>/refusal" class="btn btn-danger btn-xs">거절</a>  
          </td>
        </tr>
                        <?
                    }
